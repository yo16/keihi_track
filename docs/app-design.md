...
| メール送信 | Resend（resend パッケージ） | 経費通知メール送信 |
...
│   ├── layout.tsx                # ルートレイアウト
│   ├── page.tsx                  # トップ（ログイン＋組織作成）
│   │
│   ├── set-password/
│   │   └── page.tsx              # パスワード設定（招待ユーザー用）
│   │
│   ├── auth/
│   │   └── callback/
...
│   │   ├── layout.tsx            # 認証済みレイアウト（ヘッダー + サイドバー）
│   │   ├── dashboard/
│   │   │   └── page.tsx          # ダッシュボード（ロール別表示）
│   │   ├── expenses/
...
│       ├── organizations/
│       │   └── signup/
│       │       └── route.ts      # POST: 組織作成+ユーザー作成
│       ├── organization/
│       │   └── route.ts          # GET: 自分の組織情報
│       ├── me/
│       │   └── route.ts          # GET: 自分のメンバー情報（orgId, ロール等）
│       ├── members/
...
│   ├── auth/
│   │   ├── login-form.tsx        # ログインフォーム（メール+パスワード）
│   │   └── set-password-form.tsx # パスワード設定フォーム（招待ユーザー用）
│   ├── expenses/
...
ユーザー
  ...
  │   │
  │   ├─ 成功 → GET /api/me でロール・orgId取得
  │   │         └─ /dashboard へリダイレクト
  │   │
  │   └─ 失敗 → エラー表示
  │
  └─ 新規組織作成
      │ メールアドレス + パスワード + 組織名 + 表示名入力
      ↓
      POST /api/organizations/signup
      │ サーバー: Admin API でユーザー作成（メール確認スキップ）+ 組織作成
      ↓
      クライアント: signInWithPassword() でセッション取得
      │
      └─ /dashboard へリダイレクト
```

組織IDの入力は不要。1メール=1組織のため、認証ユーザーの `organization_members` レコードから自動的に組織を特定する。
...
|-------------|-----------|
| `/` | 公開（未認証でもアクセス可） |
| `/set-password` | 公開（招待トークンで認証） |
| `/auth/callback` | 公開（トークン処理） |
| `/dashboard` | 認証必須 |
| `/expenses/**` | 認証必須 |
| `/approvals` | 認証必須 + 承認者以上 |
| `/reports` | 認証必須 + 承認者以上 |
| `/admin/**` | 認証必須 + 管理者 |
| `/notifications` | 認証必須 |
...
リクエスト
  ...
  ├─ middleware.ts
  │   ├─ Supabaseセッションリフレッシュ
  │   ├─ 公開パス（/, /set-password, /auth/callback） → そのまま通過
  │   └─ 保護パス
  │       ├─ 未認証 → / へリダイレクト
  │       └─ 認証済み → そのまま通過
  │
  └─ 認証済みレイアウト（(authenticated)/layout.tsx）
      ├─ GET /api/me でユーザー情報・ロール・orgId取得
      ├─ 未認証（APIエラー） → / へリダイレクト
      ├─ ロール不足 → /dashboard へリダイレクト
      └─ OK → AuthContextに情報を提供 → ページ描画
```
...
ルートレイアウト（app/layout.tsx）
  ...
  │
  ├─ 公開ページ（/, /set-password, /auth/callback）
  │   └─ レイアウトなし（各ページ独自）
  │
  └─ 認証済みレイアウト（app/(authenticated)/layout.tsx）
       │ - GET /api/me でユーザー情報+ロール+orgId取得
       │ - 未認証なら / へリダイレクト
       │ - AuthContextに orgId, ユーザー情報, ロールを提供
       │ - NotificationContextを提供
...
全API Routeは以下の共通パターンで実装する。orgIdはURLパラメータからではなく、認証ユーザーの `organization_members` レコードからサーバーサイドで自動特定する。

```typescript
// app/api/expenses/route.ts の例
...
export async function POST(request: NextRequest) {
  ...
  // 2. 組織メンバー + ロールチェック（orgIdはユーザーから自動特定）
  const member = await getMemberOrFail(supabase, user.id)
  // getMemberOrFail は organization_members からレコードを取得し、
  // orgId・ロール情報を含むメンバー情報を返す。
  // 所属組織が見つからない場合は適切なエラーレスポンスをthrowする。
  ...
  const result = await createExpense(member.organization_id, user.id, parsed.data)
...
| 関数 | 説明 |
|------|------|
| `getMemberOrFail(supabase, userId)` | organization_membersからユーザーの所属組織+ロールを取得。見つからなければ403 |
...
`getMemberOrFail` はorgIdをパラメータとして受け取らず、ユーザーIDのみで所属組織を特定する（1メール=1組織の制約による）。
...
```
/ (トップ / ログイン)
├─ 組織作成 → POST /api/organizations/signup → signIn → /dashboard
└─ ログイン成功 → GET /api/me → /dashboard

/set-password (招待ユーザー用パスワード設定)
└─ パスワード設定完了 → / へリダイレクト → ログイン → /dashboard

/dashboard
├─ [使用者] → /expenses（申請一覧）
├─ [承認者] → /approvals（承認待ち一覧）
└─ [管理者] → /admin/members（ユーザー管理）

/expenses
├─ 新規申請 → /expenses/new → 申請完了 → /expenses
├─ 詳細確認 → /expenses/{id}
│   ├─ [却下済 + 本人] → 編集・再申請
│   └─ [申請中 + 本人] → 取り下げ
└─ 通知ベル → /notifications
...
- パスワード入力フィールド
- ログインボタン
- 「新規組織を作成」リンク → 組織名・メールアドレス・パスワード・表示名入力ダイアログ
...
- パスワード確認入力
- 設定ボタン
- 設定完了後、トップページ（`/`）へリダイレクト

#### 経費申請フォーム（`/expenses/new`）
...
#### ユーザー管理（`/admin/members`）

- メンバー一覧テーブル: 表示名、メールアドレス、ロール、ステータス
- 「メンバー追加」ボタン → メンバー招待フォーム
  - メールアドレス、表示名、ロール選択（パスワードは不要：招待メール経由で設定）
  - 作成完了後、招待メール送信完了メッセージを表示
...
| コンテキスト | 提供場所 | 内容 |
|-------------|---------|------|
| AuthContext | `(authenticated)/layout.tsx` | ユーザー情報、ロール、orgId（サーバーから取得） |
| NotificationContext | `(authenticated)/layout.tsx` | 未読通知数、ポーリング制御 |

AuthContextのorgIdはURLパラメータからではなく、`GET /api/me` のレスポンスから取得してContextに格納する。`useOrg` フックはこのContextからorgIdを取得する。
...
注: メール通知・メール招待は初期リリースに含まれる（Resend + Supabase Auth）。
